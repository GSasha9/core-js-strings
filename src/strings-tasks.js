...
function countVowels(str) {
  return str.split('').filter((el) => /[aeiouy]/gi.test(el)).length;
}
...
function isPalindrome(str) {
  ...
  const strRev = cleanStr.split('').reverse().join('');
  ...
  const givenArr = str
    ...
    .map((el) =>
      el
        .split('')
        .map((letter) =>
          arrUsuall.includes(letter) ? arrUsuall.indexOf(letter) : letter
        )
    );
  return givenArr
    .map((el) =>
      el.map((letter) => arrEncodeToRot13[letter] || letter).join('')
    )
    .join(' ');
}
